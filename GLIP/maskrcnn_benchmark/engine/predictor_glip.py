...
class GLIPDemo(object):
    def __init__(self,
        ...
        self.transforms_batch = self.build_transform_batch()

        # used to make colors for each token
    ...
    def build_transform(self):
        ...
        to_bgr_transform = T.Lambda(lambda x: x[[2, 1, 0], :, :])
        scale_transform = T.Lambda(lambda x: x * 255)

        normalize_transform = T.Normalize(
            mean=cfg.INPUT.PIXEL_MEAN, std=cfg.INPUT.PIXEL_STD
        )
        # print('mean ', cfg.INPUT.PIXEL_MEAN, ' std ', cfg.INPUT.PIXEL_STD)
    ...
    def build_transform_batch(self):
        """
        Creates a basic transformation that was used to train the models
        """
        cfg = self.cfg
    ...
    def run_on_web_image(self,
        ...
        image = transform_v(original_image)
        predictions, fused_features, dot_product_logits, _, _, _, _, _, _ = self.compute_prediction(image, original_caption)
        # BoxList = (bbox, size, mode, extra_fields)
        # bbox: list(tensor(size[4]), length=size), extra_fields: {'labels': (int)tensor(size[9]), 'scores': (float)tensor(size[9])}
        # bbox 가 box 좌표 4개 (top_left_xy, bottom_right_xy) / label 은 해당 box 의 label index / score 는 해당 box 의 해당 label 에 대한 score value
        top_predictions = self._post_process(predictions, thresh)
        # 위 predictions 에서 scores 가 thresh = 0.5 이상 되는 것만 top_predictions
        ...
        return result, top_predictions, predictions, fused_features, dot_product_logits
    ...
    def compute_prediction(self, original_image, original_caption, custom_entity=None):
        # original_image = ndarray(400, 400, 3) / max = 233
        # image = tensor(size(3, 400, 400))
        # image_list = ImageList(): self.tensors = size(1, 3, 416, 416) (stride 32, padding 16),  self.image_sizes
        image = self.transforms(original_image)  # 3, 400, 400 tensor / max = 2.2566
        ...
        # tokenized: vocabulary's id (BERT tokenizer) ex. [101, 2067, 28533, ...]
        # tokens_positive: noun inx in caption (NLP parser) ex. [[0, 8], [11, 15], [18, 32]]
        # positive_map: positive (is noun?) idx for sub-word ex. [[0, 0.5, 0.5, 0, ...], [0, 0, 0, 0, 1, 0, ...]], size(noun count, feature dimension)
        ...
        # positive_map_label_to_token: positive idx dict in positive map ex. {1: [1, 2], 2: [4], 3: [6, 7, 8]}
        ...
        # compute predictions
        with torch.no_grad():
            #  predictions, fused_visual_features, dot_product_logist = self.model(image_list, captions=[original_caption], positive_map=positive_map_label_to_token)
            predictions, visual_features, language_features, fused_visual_features, fused_language_features, \
                dot_product_logits, box_regression, box_scores, candidate_indexes, candidate_high_indexes_dot, candidate_high_indexes_score, anchors \
                = self.model(image, captions=[original_caption], positive_map=positive_map_label_to_token)
        ...
        return prediction, fused_visual_features, dot_product_logits, box_regression, box_scores, candidate_indexes, candidate_high_indexes_dot, candidate_high_indexes_score, anchors, positive_map_label_to_token

    def compute_prediction_grad_true(self, original_image, original_caption, custom_entity=None):
        # original_image = ndarray(400, 400, 3) / max = 233
        # image = tensor(size(3, 400, 400))
        # image_list = ImageList(): self.tensors = size(1, 3, 416, 416) (stride 32, padding 16),  self.image_sizes
        image = self.transforms(original_image)  # 3, 400, 400 tensor / max = 2.2566
    ...
    def compute_prediction_visualize(self, original_image, original_caption, custom_entity=None):
        # original_image = ndarray(400, 400, 3) / max = 233
        # image = tensor(size(3, 400, 400))
        # image_list = ImageList(): self.tensors = size(1, 3, 416, 416) (stride 32, padding 16),  self.image_sizes
        image = self.transforms(original_image)  # 3, 400, 400 tensor / max = 2.2566
        ...
        return prediction, visual_features, language_features, fused_visual_features, fused_language_features,\
            dot_product_logits, box_regression, box_scores, candidate_indexes, candidate_high_indexes_dot, candidate_high_indexes_score, anchors, positive_map_label_to_token
    ...
    def _post_process(self, predictions, threshold=0.5):
        ...
        thresh = scores.clone()
        for i, lb in enumerate(labels):
            if isinstance(self.confidence_threshold, float):  # use
                thresh[i] = threshold
            elif len(self.confidence_threshold) == 1:
                thresh[i] = threshold
            else:
        ...
        # iidx = []
        # for i in idx:
        #     if labels[i] not in iidx:
        #         iidx.append(i)
        # iidx = torch.stack(iidx)
        return predictions[idx]
    ...
    def overlay_entity_names(self, image, predictions, names=None, text_size=0.7, text_pixel=1, text_offset=5,
                             text_offset_original=4):
        labels = predictions.get_field("labels")
        colors = self.compute_colors_for_labels(labels).tolist()
        ...
        for box, score, label, color in zip(boxes, scores, new_labels, colors):
            ...
            cv2.putText(
                image, s, (int(x), int(y) - text_offset_original), cv2.FONT_HERSHEY_SIMPLEX, text_size, tuple(color),
