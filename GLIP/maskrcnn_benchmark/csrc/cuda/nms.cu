...
//     const int col_blocks = THCCeilDiv(n_boxes, threadsPerBlock);
    const int col_blocks = (n_boxes + threadsPerBlock) / threadsPerBlock;
...
//   const int col_blocks = THCCeilDiv(boxes_num, threadsPerBlock);
  const int col_blocks = (boxes_num + threadsPerBlock) / threadsPerBlock;
...
//   THCudaFree(state, mask_dev);
  c10::cuda::CUDACachingAllocator::raw_delete(mask_dev); // 이거 포인터 넣어줘야 함
