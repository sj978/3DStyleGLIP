...
class VLDyHead(torch.nn.Module):
    ...
    def forward(self, x, language_dict_features=None, embedding=None, swint_feature_c4=None):
        ...
        # soft token
        t_logits = None
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_TOKEN_LOSS:    # not use
            t_logits = []

        fused_language_features = []
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_FUSED_FEATURES_DOT_PRODUCT:    # use
            embedding = dyhead_tower["lang"]["hidden"]
            fused_language_features.append(embedding)
        
        # MLM loss
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.MLM_LOSS:    # not use
        ...
        proj_tokens = None
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_CONTRASTIVE_ALIGN_LOSS:    # not use
            contrastive_logits = []
            # follow MDETR's way
        ...
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_DOT_PRODUCT_TOKEN_LOSS:    # use
            dot_product_logits = []
            # norm
        ...
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_SHALLOW_CONTRASTIVE_LOSS \
                or self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_BACKBONE_SHALLOW_CONTRASTIVE_LOSS:    # not use
        ...
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_BACKBONE_SHALLOW_CONTRASTIVE_LOSS:    # not use
        ...
        for l, feature in enumerate(x):
            ...
            if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_DOT_PRODUCT_TOKEN_LOSS:    # use
                ...
                if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.CLAMP_DOT_PRODUCT:    # use
                    dot_product_logit = torch.clamp(dot_product_logit, max=50000)
                    dot_product_logit = torch.clamp(dot_product_logit, min=-50000)
                dot_product_logits.append(dot_product_logit)

            if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_SHALLOW_CONTRASTIVE_LOSS:    # not use
                feat = feature
                BF, CF, HF, WF = feat.shape
                shallow_img_emb = permute_and_flatten(feat, BF, -1, CF, HF, WF)
        ...
        if shallow_img_emb_feats is not None and shallow_text_emb is not None:    # not use
            # shallow_img_embs = torch.cat(shallow_img_embs, dim=1)
            proj_tokens = shallow_text_emb
        return logits, bbox_reg, centerness, t_logits, proj_tokens, contrastive_logits, dot_product_logits, mlm_logits, shallow_img_emb_feats, fused_visual_features, fused_language_features
...
class VLDyHeadModule(torch.nn.Module):
    ...
    def forward(self, images, features, targets=None,
        ...
        elif self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_DOT_PRODUCT_TOKEN_LOSS:    # use
            # no resizer needed
            embedding = language_dict_features['embedded']
        else:    # not use
            embedding = None

        if "masks" in language_dict_features:    # use
            text_masks = language_dict_features["masks"]
        else:
            text_masks = None
        
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.ADD_LINEAR_LAYER:    # not use
        ...
        # input (features, language_dict_features, embedding)
        # features (5 tensors tuple / [1, 256, w, h])
        # language_dict_features (dict {'aggregate', 'embedded', 'masks', 'hidden'})
        # embedding (tensor [1, 256, 768])
        box_cls, box_regression, centerness, token_logits, \
        proj_tokens, contrastive_logits, dot_product_logits, mlm_logits, shallow_img_emb_feats, fused_visual_features, fused_language_features = self.head(features,
                                                                        language_dict_features,
                                                                        embedding,
                                                                        swint_feature_c4
                                                                        )
        # output (box_cls, box_regression, centerness, dot_product_logits, fused_visual_features)
        # box_cls (5 tensors tuple / [1, 80, w, h])
        # box_regression (5 tensors tuple+list / [1, 4, w, h])
        # centerness (5 tensors tuple+list / [1, w, h])
        # dot_product_logits (5 tensors list / [1, w x h, 256])
        # fused_visual_features (5 tensors list / [1, 256, w, h])

        anchors = self.anchor_generator(images, features)
        # anchors (5-level tuple / BoxLists, bbox_count = w x h, size = (400, 400), extra_fields = 'visibility')
        ...
        if self.training:
            ...
            return self._forward_test(box_regression, centerness, anchors,
                                      ...
                                      positive_map,
                                      fused_visual_features=fused_visual_features,
                                      fused_language_features=fused_language_features
                                      )
    ...
    def _forward_test(self, box_regression, centerness, anchors,
                      ...
                      positive_map=None,
                      fused_visual_features=None,
                      fused_language_features=None
                      ):

        boxes, box_scores, candidate_indexes, candidate_high_indexes_dot, candidate_high_indexes_score = self.box_selector_test(box_regression, centerness, anchors,
                                       box_cls,
                                       token_logits,
                                       dot_product_logits,
                                       positive_map,
                                       )
        return boxes, {}, fused_visual_features, fused_language_features, dot_product_logits, box_regression, box_scores, candidate_indexes, candidate_high_indexes_dot, candidate_high_indexes_score, anchors
