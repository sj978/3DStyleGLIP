...
class ATSSLossComputation(torch.nn.Module):
    ...
    def prepare_targets(self, targets, anchors, tokenized=None, positive_map=None, proj_tokens=None):
        ...
        positive_indices = []
        offset = 0
        # target = list(Boxlist) 형태
        # len(targets)하면 1 나오는 상태

        for im_i in range(len(targets)):  # Ground truth Boxlist를 순회. Batch 단위 학습할 때는 gt가 B만큼 있을테니
            targets_per_im = targets[im_i]  # Ground truth Boxlist 하나 가져옴
            assert targets_per_im.mode == "xyxy"
            # bboxes_per_im = targets_per_im.get_field("boxes")
            bboxes_per_im = targets_per_im.bbox  # 해당 Boxlist의 박스들 bbox 불러옴
            labels_per_im = targets_per_im.get_field("labels")  # 해당 Boxlist의 labels 가져옴. label은 1부터 시작하게 되어잇음
            num_gt = len(bboxes_per_im)  # 해당 ground truth에서 bbox의 개수 - 원본 코드

            if positive_map is not None:
                # positive_map은 [2, 256]의 형태를 지님
                # 현재 num_gt랑 positive map의 0dim 크기랑 같아야 해당 코드의 인덱싱이 의도대로 잘 될 것 같은데 이러면 문제가 발생하는건가
                token_per_im = positive_map[offset:offset + num_gt, :]
                # print("token per im shape: ", token_per_im.shape)
                # token_per_im shape - [num_gt, 256]
                offset += num_gt
                # print("num_gt: ", num_gt)

            # Recheck if the label matches with the positive map
            # print("Checking if the label matches with the positive map: ")
            # print(labels_per_im)
            ...
            # anchors는 [[Boxlist, Boxlist, Boxlist, Boxlist, Boxslist]]임. 즉, Boxlist 인스턴스 5개를 1개의 리스트가 감싸고 있고, 그것 역시 외부의 리스트로 감싸져 있는 형태 [1, 5]
            anchors_per_im = cat_boxlist(anchors[im_i]) # 여러 Boxlist를 하나의 Boxlist로 합치는 함수
            # im_i는 batch의 인덱스이므로, 해당 인덱스에 해당하는 anchors를 가져다가 하나의 박스리스트로 합치는 것 같다
            # anchors per im은 하나의 Boxlist 인스턴스임. 5개의 Boxlist들의 bbox들이 다 concat된 모습인듯
            ...
            # targets per im은 target이라는 이름으로 받아오는 GT Boxlist중 현재 i번째 배치에 해당되는 녀석의 Boxlist
            # anchors per im은 VIT의 visual features와 image를 통해 생성해낸 Boxlist 중 현재 i번째 배치에 해당되는 녀석의 Boxlist
            ious = boxlist_iou(anchors_per_im, targets_per_im)
            # 그 둘 간의 iou를 계산 - Compute the intersection over union of two set of boxes
            # 뭐 엄청 복잡한건 아니고 두 Boxlist간의 area 간의 비교를 통해 계산하는듯
            # ious는 tensor로 나오고 [2586, num_gt]의 형태임. 여기서 num_gt는 현재 GT Boxlist의 bbox 갯수를 의미하고 2586은 이미지 크기에 비례한 값
            # ious 내부의 minimum 값은 0이고 가장 큰 값은 보통 0.7~0.76 정도의 값을 가짐
            # ious 비교 코드 분석은 실패
            ...
            # ious가 [2586, num_gt] 형태였던 것을 -INF로 채운 뒤 transpose하고 Flatten한 형태 - [2586 x num_gt]
            index = candidate_idxs.view(-1)[is_pos.view(-1)]
            # print("index: ", index)
            ious_inf[index] = ious.t().contiguous().view(-1)[index]
            ious_inf = ious_inf.view(num_gt, -1).t()
            # print("final ious_inf shape: ", ious_inf.shape)
            # 최종적인 ious inf의 형태는 [2586, num_gt]로 나온다

            anchors_to_gt_values, anchors_to_gt_indexs = ious_inf.max(dim=1) # max의 value와 해당 value의 index 값이 담긴 tensor 두개가 나옴
            # anchors to gt indexs는 [2586]의 형태임
            # setter = set()
            # for i in anchors_to_gt_indexs:
            #     setter.add(int(i))
            # print("setter: ", setter)
            ...
            if positive_map is not None:
                # token per im shape: [noun, 256]
                # 여기서 anchors to gt indexs는 [2586]인데, 그 중 element의 값이 noun 개수 이상인 것이 존재하고 있어서 에러남
                # token labels per im 은 token per im 으로부터 i번째 row를 통째로 뜯어와서 [2586, 256]이 되는 구조임
                token_labels_per_im = token_per_im[anchors_to_gt_indexs]

                unmatched_labels = torch.zeros(token_labels_per_im.shape[1],
                                               device=token_labels_per_im.device)  # 1 번째를 뽑는 걸로 봐서 2 dimension을 생각하는듯
    ...
    def __call__(self, box_cls, box_regression, centerness, targets, anchors,
        ...
        # print("Token labels stacked!!!!!")
        # print(token_labels_stacked.shape)
        # for i in range(2586):
        #     print("i{}th row: ".format(i), torch.nonzero(token_labels_stacked[0][i]))
        # print(token_labels_stacked)
        # print("-----------------------End----------------------")
