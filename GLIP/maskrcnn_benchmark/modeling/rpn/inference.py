...
class ATSSPostProcessor(torch.nn.Module):
    ...
    def forward_for_single_feature_map(self, box_regression, centerness, anchors,
        ...
        if dot_product_logits is not None:
            ...
            # 요 부분이 dot_product_logits 이 logits 으로 행동하기 위해 sigmoid 가 붙는 부분
            if self.mdetr_style_aggregate_class_num != -1:  # not use
                ...
                # 이 부분이 중요
                # dot_product_logits 기반으로 score 를 어떻게 뽑아내는지
                # 결론은 dot_product_logits 에서 noun 에 해당하는 부분만 가져오되, sub-words 로 이루어져 있는 noun 의 경우 평균을 내서 가져온다.
                # (1, w x h, 80) 의 tensor 로, 80 중 noun_count 의 idx 부분만 value 가 채워져 있고, 나머지는 0 으로 채워진다.
                # 여기서 80 은 pre-defined class 개수이다.
        ...
        box_regression = permute_and_flatten(box_regression, N, A, 4, H, W)
        box_regression = box_regression.reshape(N, -1, 4)
        # print('scores', box_cls.shape)
        # print(box_cls[0][0][:])
        # print('box_regression', box_regression.shape)

        candidate_inds = box_cls > self.pre_nms_thresh

        # print(candidate_inds[0][0][:])
        pre_nms_top_n = candidate_inds.reshape(N, -1).sum(1)
        ...
        candidate_inds_high_dot = box_cls > 0.25
        # multiply the classification scores with centerness scores
        box_cls = box_cls * centerness[:, :, None]
        candidate_inds_high_score = box_cls > 0.25

        results = []

        # 단순히 batch 푸는 과정 (어차피 batch 는 1)
        # w x h = 2704
        # per_box_cls: [w x h, 80]
        # per_box_regression: [w x h, 4]
        # per_pre_nms_top_n: [top_개수]
        # per_candidate_inds: [w x h, 80], True, False 저장되어 있음
        # per_anchors: list, len=w x h
        for per_box_cls, per_box_regression, per_pre_nms_top_n, per_candidate_inds, per_anchors \
            ...
            per_box_cls = per_box_cls[per_candidate_inds]
            # 여기서 per_box_cls 는 score 이다.
            # per_box_cls: 2704 개 중 nms_top_n 개수, 즉 candidate_idx 만 살아 남은 친구들, 즉 threshold 를 넘는 dot_product 값을 갖고 있는 친구들

            per_box_cls, top_k_indices = per_box_cls.topk(per_pre_nms_top_n, sorted=False)
            # pre-defined nms top 개수가 1000 개, 그거 넘으면 1000 개로 제한하는 역할만 함. 별거 없음

            per_candidate_nonzeros = per_candidate_inds.nonzero()[top_k_indices, :]
            # candidate 의 index, label 표시된 tensor

            # 위 per_candidate_nonzeros 에서 각각 candidate box 의 index 와 class 값을 가져옴
            per_box_loc = per_candidate_nonzeros[:, 0]
            per_class = per_candidate_nonzeros[:, 1] + 1

            # print('inner')
            # print(per_box_loc.shape) -> torch.Size([50])
            # print(per_box_regression.shape) -> torch.Size([1600, 4])
            # print(per_box_regression[per_box_loc, :].shape) -> torch.Size([50, 4])
            # print(per_anchors) -> BoxList(num_boxes=1600, image_width=320, image_height=320, mode=xyxy)
            # print(per_anchors.bbox[per_box_loc, :].shape) -> torch.Size([50, 4])

            # 위에서 candidate 들은 모두 box encoding 이므로 decoding 해서 box 의 top left, bottom right 좌표 값으로 변환
            detections = self.box_coder.decode(
                per_box_regression[per_box_loc, :].view(-1, 4),
                per_anchors.bbox[per_box_loc, :].view(-1, 4)
            )
        ...
        return results, box_cls, candidate_inds, candidate_inds_high_dot, candidate_inds_high_score
    ...
    def forward(self, box_regression, centerness, anchors,
        ...
        box_scores = []
        candidate_indexes = []
        candidate_high_indexes_dot = []
        candidate_high_indexes_score = []
        anchors = list(zip(*anchors))
        for idx, (b, c, a) in enumerate(zip(box_regression, centerness, anchors)):
            ...
            # )
            r, bc, ci, ci_dot, ci_score = self.forward_for_single_feature_map(b, c, a, o, t, d, positive_map)
            sampled_boxes.append(r)
            box_scores.append(bc)
            candidate_indexes.append(ci)
            candidate_high_indexes_dot.append(ci_dot)
            candidate_high_indexes_score.append(ci_score)

        boxlists = list(zip(*sampled_boxes))
        boxlists = [cat_boxlist(boxlist) for boxlist in boxlists]
        if not (self.bbox_aug_enabled and not self.bbox_aug_vote):  # use
            boxlists = self.select_over_all_levels(boxlists)

        return boxlists, box_scores, candidate_indexes, candidate_high_indexes_dot, candidate_high_indexes_score
    ...
    if positive_map is not None:
        ...
        if score_agg == "MEAN":
            for label_j in positive_map:
                # scores 를 구하는 방식은
                # 1. noun 의 위치를 (256에서) positive map 에서 뽑아온다.
                # 2. dot_product_logits 에서 noun 의 value 를 가져오고, 만약 noun 이 여러 sub-noun 들의 합이라면 이들을 평균낸다.
                # 3. scores 는 dot_product_logits (1, 2704, 256) 에서 noun 에 해당하는 logits (1, 2704, noun_count) 을 뽑아서 class 가 80개라고 가정한 zero_tensor 에 저장한다. (1, 2704, 80)
