...
def main():
    ...
    if output_dir:
        mkdir(output_dir)
    logger = setup_logger("maskrcnn_benchmark", output_dir, get_rank(), cfg.DATASETS.MEMO)
    ...
    for task_id, ft_cfg in enumerate(ft_configs, 1):
        for shuffle_seed in shuffle_seeds:
            ...
            ft_output_dir = ft_output_dir + "/{}".format(cfg.DATASETS.DATASET_NAME)

            Path(ft_output_dir).mkdir(parents=True, exist_ok=True)
